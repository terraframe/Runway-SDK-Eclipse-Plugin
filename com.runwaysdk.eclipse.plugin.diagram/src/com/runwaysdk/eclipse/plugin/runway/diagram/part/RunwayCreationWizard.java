...
public class RunwayCreationWizard extends Wizard implements INewWizard
{
    ...
    domainModelFilePage = new RunwayCreationWizardPage("DomainModelFile", getSelection(), "runway") { //$NON-NLS-1$ //$NON-NLS-2$
      ...
      public void setVisible(boolean visible)
      {
        if (visible)
        {
          ...
          setFileName(RunwayDiagramEditorUtil.getUniqueFileName(getContainerFullPath(), fileName,
              "runway")); //$NON-NLS-1$
        }
  ...
  public boolean performFinish()
  {
    IRunnableWithProgress op = new WorkspaceModifyOperation(null)
    {
      ...
      {
        diagram = RunwayDiagramEditorUtil.createDiagram(diagramModelFilePage.getURI(),
